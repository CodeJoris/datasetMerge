...
def parse_filename(filename, sid_map, next_sid):
    ...
    Args:
        filename (str): The name of the file (e.g., 'Accelerometer-2011-03-24-10-24-39-climb_stairs-f1.txt').
        sid_map (dict): A dictionary mapping original volunteer IDs (e.g., 'f1') to integer SIDs.
        next_sid (int): The next available integer to assign for a new volunteer ID.

    Returns:
        tuple: (surface (str), sid (int), updated_next_sid (int)) or (None, None, next_sid) if parsing fails.
    """
...
def convert_acceleration_to_ms2(df):
    ...
    Args:
        df (pd.DataFrame): The dataframe containing raw coded 6-bit acceleration data in columns 0, 1, and 2.

    Returns:
        pd.DataFrame: The dataframe with converted acceleration values.
    """
    for col in range(3):
        df[col] = (-1.5 + (df[col] / 63.0) * 3.0) * G_TO_MS2
    return df

def format_dataframe(df, surface, sid):
    """
    Adds required metadata columns and renames existing columns to match the unified standard.

    Args:
        df (pd.DataFrame): The dataframe containing the converted x, y, and z acceleration data.
        surface (str): The standardized surface name.
        sid (int): The standardized subject ID.

    Returns:
        pd.DataFrame: The fully formatted dataframe ready for concatenation.
    """
    ...
    df = df[['time_ms', 'sid', 'surface', 0, 1, 2]]
    df.columns = [
        'time_ms', 
        'sid', 
        'surface', 
...
def process_single_file(file_path, filename, sid_map, next_sid):
    """
    ...
    Args:
        ...
        sid_map (dict): The current mapping of volunteer IDs to integer SIDs.
        next_sid (int): The next available integer SID.

    Returns:
        tuple: (processed_dataframe or None, updated_next_sid)
    """
    ...
    df = format_dataframe(df, surface, sid)
...
def process_bruno_dataset(base_dir, output_csv):
    ...
    Args:
        base_dir (str): The root directory of the dataset (e.g., './datasets/bruno1').
        output_csv (str): The desired filename/path for the exported CSV file.
    """
    data_frames = []
    sid_map = {}
    next_sid = 1
    ...
    for root, _, files in os.walk(base_dir):
        ...
        for file in files:
            ...
            df, next_sid = process_single_file(file_path, file, sid_map, next_sid)
