...
def print_sensor_locations(file_path: Path):
    ...
    """
    try:
        df = pd.read_csv(file_path, usecols=['sensor_location'])  # Read only the sensor_location column
        unique_locations = df['sensor_location'].dropna().unique()
        print(f"Unique sensor locations in {file_path.name}:")
