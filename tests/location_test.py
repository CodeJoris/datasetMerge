...
def print_sensor_locations(file_path: Path):
    """
    Prints the unique sensor locations present in a CSV file.
    ...
    try:
        ...
        # In column name. eg column name: "left_thigh_EMG" or "trunk_Acc_x"
        unique_metrics = set()
        for col in df.columns:
            if col in ["time_ms", "sid", "surface", "trial_no", "packetcounter"]:
                continue  # Skip metadata columns
            split0 = col.split('_')[0]
            split1 = col.split('_')[1]
            unique_metrics.add(f'{split0}_{split1}' if split0 in ['left', 'right'] else split0)  # Extract the metric part
